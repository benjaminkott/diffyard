...
export function shell(result: RunResult, config: Config, data: string): string {
  ...
  return `<!doctype html>
<html lang="en" data-theme="auto" data-view="overview" data-mode="${smoke ? 'smoke' : 'compare'}">
<head>
<meta charset="utf-8">
...
<header class="topbar">
  <div class="topbar__title">
    ...
    ${smoke ? site(result) : result.config.a && result.config.b ? sides(result) : ""}
...
<div class="controls">
  <div class="filters" role="group" aria-label="Filter ${smoke ? 'pages' : 'comparisons'}">
    ${smoke ? smokeFilters(result) : compareFilters(result)}
  </div>
  ${smoke ? '' : kindFilters(result)}
  <label class="sort sort--overview">
    <span>Sort</span>
    <select id="sort">
      <option value="diff">${smoke ? 'Failed first' : 'Largest difference'}</option>
...
<section class="overview" id="overview">
  <div class="overview__head">
    ...
    <p class="overview__hint">
      ${smoke
        ? 'One tile per page. The picture is the page as it was captured, and the rows ' +
          'are how it answered, per viewport. Select one to open it.'
...
<section class="detail" id="detail" hidden>
  <div class="detail__bar">
    ...
    <div class="modes" id="modes" role="group" aria-label="View mode">
      ${smoke
        ? '<button type="button" data-mode="page" class="is-active">Page</button>' +
...
<!--
  What the line has to do is say what this page is, to someone at the foot of
  it. "Comparing two URLs" is the input, not the work, and it reads small
  against nine hundred comparisons across twenty sites -- the two URLs are only
  how the two systems were named. It also undersold the report: what is on
  these pages is pixels, markup and console output.
-->
<footer class="colophon">
  <p>Generated by <a href="${escapeHtml(HOMEPAGE)}" target="_blank" rel="noreferrer noopener">diffyard</a>
    <span class="colophon__version">${escapeHtml(VERSION)}</span> — ${smoke
      ? 'every page of one site, and whether it came back clean.'
      : 'the same pages on two systems, and everything that differs.'}</p>
</footer>
...
function compareFilters(result: RunResult): string {
  return (
    ...
    (answersDiffer(result) > 0
      ? `<button type="button" data-filter="answer" class="is-warning">Answered differently (${answersDiffer(result)})</button>`
      : '') +
    (consoleDiffers(result) > 0
      ? `<button type="button" data-filter="console">Console differs (${consoleDiffers(result)})</button>`
      : '')
  );
}

/**
 * The questions a smoke run answers: which pages failed, which captures
 * broke, and which pages are not where the config says they are. A redirect
 * is not a failure, but a page that moved is worth knowing about, and left
 * out the list would have nowhere to say so.
 */
function smokeFilters(result: RunResult): string {
  const moved = result.comparisons.filter((entry) => entry.smoke?.answer.redirected).length;
  return (
    `<button type="button" data-filter="all" class="is-active">All (${result.total})</button>` +
    `<button type="button" data-filter="fail">Failed (${result.failed})</button>` +
    `<button type="button" data-filter="error">Errors (${result.errored})</button>` +
    (moved > 0 ? `<button type="button" data-filter="redirect">Redirected (${moved})</button>` : '')
  );
}

/** The one site a smoke run walked, where a comparison names its two. */
function site(result: RunResult): string {
  if (!result.config.a) return '';
  const label = result.config.labelA && result.config.labelA !== 'A'
    ? `<span class="urls__label">${escapeHtml(result.config.labelA)}</span> `
    : '';
  return `<p class="urls">${label}<code>${escapeHtml(result.config.a)}</code></p>`;
}
...
function formatMeta(result: RunResult): string {
  ...
  const what = result.mode === 'smoke' ? 'pages' : 'comparisons';
  return `run ${result.runId} · ${result.total} ${what} · ${result.config.browser} · ${seconds}s · ${started}`;
}
